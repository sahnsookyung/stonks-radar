...
function SignalLink({ signal, locale }: Readonly<{ signal: TickerSignal; locale: "en" | "ko" }>) {
  const sourceHref = safeExternalUrl(signal.item.source_url);
  const sourceIcon = sourceHref ? (
...
function TransactionRow({ transaction, locale }: Readonly<{ transaction: DisclosureTransaction; locale: "en" | "ko" }>) {
  ...
  const className = "focus-ring grid gap-3 rounded-md border border-line bg-panelAlt p-4 hover:border-accent md:grid-cols-[minmax(0,1fr)_160px_150px]";
  const content = (
    <>
      <div className="min-w-0">
        <div className="flex flex-wrap gap-2">
