...
export function MarketPulseTickerBar({ tiles }: Readonly<{ tiles: MetricTile[] }>) {
  ...
  return (
    <section
      ...
      aria-labelledby="market-pulse-strip-title"
    >
      <div className="mb-2 flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            type="button"
            className="secondary-action h-11 min-h-11 w-11 p-0"
            onClick={() => scroll(-1)}
    ...
    <div className="grid min-w-0 gap-6">
      ...
      {groups.map((group) => (
        <section
          key={group.key}
          className="min-w-0"
          aria-labelledby={`market-pulse-${group.key}`}
        >
          <h2
            id={`market-pulse-${group.key}`}
  ...
  const body = (
    <>
      ...
      <div className="min-w-0 flex-1">
        <div className="truncate text-xs font-semibold leading-4 text-muted">
          {tile.label}
        </div>
        ...
        <div
          className={`mt-0.5 truncate text-[11px] font-medium leading-4 ${update.toneClass}`}
        >
          {update.label}
        </div>
      </div>
    ...
    <>
      ...
      <div className="mt-3 h-24 shrink-0 sm:h-28">
        {tile.points && tile.points.length >= 2 ? (
          ...
          <div
            className="h-full rounded-md border border-line bg-paper/40"
            aria-hidden="true"
          />
        )}
      </div>
      <div className="mt-3 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs leading-5 text-muted">
        <span className={`font-semibold ${update.toneClass}`}>
          {update.label}
        </span>
...
function DeltaBadge({
  ...
  if (Math.abs(delta) < 0.000001) {
    return (
      <span
        className={`shrink-0 font-semibold text-muted ${compact ? "text-xs" : "text-sm"}`}
      >
        0
      </span>
    );
  ...
  return (
    <span
      className={`inline-flex shrink-0 items-center gap-0.5 font-semibold ${tone} ${compact ? "text-xs" : "text-sm"}`}
    >
...
function TinySparkline({
  ...
  if (points.length < 2) {
    return (
      <div
        className="h-full rounded border border-line bg-paper/40"
        aria-hidden="true"
      />
    );
  }
...
function groupMarketTiles(tiles: MetricTile[], locale: "en" | "ko") {
  ...
  for (const tile of tiles) {
    if (tile.key.startsWith("us_") || tile.key.startsWith("japan_"))
      groups[2].tiles.push(tile);
    else if (
      tile.key.startsWith("usd_") ||
      tile.key.includes("crude") ||
      tile.key.endsWith("_futures")
    )
      groups[1].tiles.push(tile);
...
function formatRefreshInterval(seconds: number, locale: "en" | "ko") {
  if (seconds < 60)
    return locale === "ko" ? `${seconds}초마다` : `every ${seconds}s`;
...
function metricUpdate(tile: MetricTile, locale: "en" | "ko", now: number) {
  ...
  const marketClose = isMarketCloseObservation(tile);
  const graceSeconds = marketClose
    ? 72 * 60 * 60
    : Math.max((refreshSeconds ?? 0) * 2, 20 * 60);
  const isOverdue =
    typeof refreshSeconds === "number" && now - updatedAt > graceSeconds * 1000;
...
function isMarketCloseObservation(tile: MetricTile) {
  if (tile.freshness === "fresh") return false;
  const source = `${tile.source} ${tile.delay_label}`.toLowerCase();
  return (
    source.includes("quote") ||
    source.includes("futures") ||
    source.includes("fx") ||
    source.includes("market may be closed")
  );
}

function formatMetricUpdateAge(
  ...
  if (elapsedMinutes < 60)
    return locale === "ko"
      ...
      : `${prefix}${elapsedMinutes}m ago`;
  const elapsedHours = Math.floor(elapsedMinutes / 60);
  if (elapsedHours < 48)
    return locale === "ko"
      ? `${prefix}${elapsedHours}시간 전`
      : `${prefix}${elapsedHours}h ago`;
