...
| FINRA Query API | configured, missing, disabled, or quota-limited credential state | OAuth client credentials; synchronous: 1,200 requests/minute/IP, 5,000 records/request, 3 MB sync response; async: 20 requests/minute/dataset/API account; credential data limit is 10 GB/month | Reg SHO once after 6:00 p.m. ET and once next morning; short interest twice monthly plus one daily retry window after release dates. |
