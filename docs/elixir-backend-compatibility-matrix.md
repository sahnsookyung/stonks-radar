...
This matrix is the implementation gate for the Phoenix/Ecto/Oban backend.
React/Vite stays unchanged, Caddy continues to own `/public/*`, and Phoenix must
preserve the HTTP, cookie, cache, and JSON contracts the frontend already uses.
...
All `/api/*` Phoenix responses must keep the established security and CORS
...
| `admin.refresh_instruments` | `POST /api/admin/instruments/refresh` | `AdminDashboard` | `200` `{"status":"refreshed","job_id":...,"refresh":...}`; Elixir job IDs use `oban:<id>`. | Owner/admin plus CSRF; audit write required; Oban enqueue fixture required. | L1 |
| `admin.ingest_url` | `POST /api/admin/ingest/url` | admin source ops | `200` document `id`; `400` SafeFetch/source ingestion error detail. | Owner/admin/editor plus CSRF; arbitrary URL reads must go through Elixir SafeFetch. | L1 |
...
| `instruments.search` | `GET /api/instruments/search` | `PortfolioLabPage` | `200`; supports trimmed `q`, `limit=1..25`, country/exchange/asset filters, advanced/inactive flags, and context; validation errors keep the legacy-compatible shape. | Public route; JSON, common headers, `Cache-Control=no-store`, no cookies; rate limit policy preserved. | L1 |
...
| `static.latest_manifest` | `GET /public/latest/manifest.json` | React public snapshot loader | `200` static manifest from published snapshot volume. Phoenix must not own this route. | Caddy-owned `Cache-Control=no-cache, max-age=0, must-revalidate`; `/public/*` keeps short public cache. | L2 |

## Fixture Strategy

Do not capture from live services in CI. Fixture generation is a local, explicit
developer task against deterministic local data, never production.

1. Exercise each matrix row with deterministic inputs from this file.
2. Store only sanitized request/response pairs under
   `apps/backend_elixir/test/support/fixtures/compatibility/<row-id>.json`.
3. Include status, selected response headers, response cookies metadata, and
...
4. Phoenix contract tests should load fixtures with
...
5. Rows involving Oban, snapshot files, SafeFetch, OAuth, or email webhooks must
...
| Surface | Guardrail |
| --- | --- |
| npm | `backend:test:contract` runs the Mix contract alias from repo root. `backend:check` fetches Elixir deps, compiles Phoenix, and runs the contract gate. Root `test` runs React unit tests plus the Elixir contract gate; `test:all` expands to full Elixir tests and Playwright. |
| Mix | `mix test.contract` runs tests tagged `:contract`, including DB-free and fixture-backed HTTP contracts. Non-contract Phoenix controller/domain tests remain under `backend:test`. |
| Compose dev | `api-elixir` is the backend service on port `8000`; `compose.dev.yaml` only exposes Postgres, Valkey, and the web dev server for local use. |
| Compose prod | Caddy proxies `/api/*` to `api-elixir:8000`. The service must expose a container healthcheck on `/api/public/health`. |
| Published snapshots | `published-snapshots` volume remains writable by the Elixir backend and read-only by Caddy. Contract tests must use temp fixtures, not the real production volume. |
...
- React/Vite source and generated public snapshots are unchanged by backend
  maintenance, except for separately approved generated fixture updates.
- Oban is the only durable executor; legacy `job_queue` rows are frozen after
  migration and surfaced only through admin projections/replay compatibility.
- Arbitrary URL ingestion uses Elixir SafeFetch with SSRF, redirect, timeout,
  content, and byte-cap protections.
- Staging and production prove unchanged admin login, CSRF, source admin,
  portfolio lookup, market history, snapshot publish/rollback, email webhook,
  and public static manifest flows.
- Production go/no-go requires `docker compose config`, `mix test.contract`,
  `backend:check`, and a live smoke that checks `/api/public/health`,
  `/api/auth/google/config`, and `/public/latest/manifest.json`.
