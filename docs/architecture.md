...
FastAPI is used for authenticated owner/admin/editor workflows: ingestion, review, source policy, job queue, publication, quotas, and audit. PostgreSQL is canonical. Valkey/Redis is only for cache, locks, or rate counters.

Durable work uses `job_queue` with leases, heartbeats, dependency checks, `FOR UPDATE SKIP LOCKED`, retry backoff, dead letters, replay, and concurrency caps.
...
Snapshot publication is local to the OCI host. API/worker containers write validated candidate and published JSON into Docker volumes, and Caddy serves the published volume at `/public/...`.

Deployment uses a guarded production compose profile: only Caddy publishes host
ports `80/443`; Postgres, Valkey, API, worker, fetch sandbox, and web stay on
